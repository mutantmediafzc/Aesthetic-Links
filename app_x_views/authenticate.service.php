...
if ($stmt = $con->prepare('SELECT id, password, userlevel FROM accounts WHERE email = ?')) {
	...
	if (password_verify($_POST['password'], $password)) {
		...
		header('Location: book-now-discount.php?cunique=' . $cunique . '&id=' . $cid . '&keydate=' . $keydate . '&keytime=' . $keytime . '&keyexpert=' . $keyexpert . '&keynote=' . $keynote);
	} else {
		// Incorrect password
		header('Location: login.incorrect.services.php?cunique=' . $cunique . '&id=' . $cid . '&date=' . $keydate . '&time=' . $keytime . '&expert=' . $keyexpert . '&note=' . $keynote);
