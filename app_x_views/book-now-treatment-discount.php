...
?>
<?php
$keydate = htmlspecialchars($_GET['keydate'] ?? null);
$keytime = htmlspecialchars($_GET['keytime'] ?? null);
$keyexpert = htmlspecialchars($_GET['keyexpert'] ?? null);
$keynote = htmlspecialchars($_GET['keynote'] ?? null);

// If the user is not logged in redirect to the login page...
if (!isset($_SESSION['loggedin'])) {
    header('Location: login.blade.treatment2.php?cunique=' . $cunique . '&id=' . $id . '&date=' . $keydate . '&time=' . $keytime . '&expert=' . $keyexpert . '&note=' . $keynote);
    exit;
}
?>
...
<div id="discountModal" class="discountmodal">
    ...
    <div class="time-content" style=" display: grid; grid-template-columns: repeat(1, 1fr);">
        ...
        <div class="spacer-modal" style="display:flex; justify-content: start; align-items: center;">
            <h1 style="margin-top: 20px; margin-left: 20px; white-space: nowrap;">Enter discount code</h1>
        </div>
