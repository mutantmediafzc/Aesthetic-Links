...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $query = "SELECT * FROM biddings WHERE bidding_event_id='" . $row["id"] . "' AND status = 'complete' AND clinic_country = 'Switzerland'";
...
<div>
    ...
    <div class="result featured">
            <?php
            $stmt = "SELECT c.*, COUNT(r.id) as total_reviews, AVG(r.rating) AS average_rating FROM clinics c LEFT JOIN reviews r ON c.cunique = r.cunique COLLATE utf8mb4_unicode_ci WHERE c.approval = 'approved' AND c.ccountry = 'Switzerland' AND c.id = {$clinicId} GROUP BY c.cunique";
