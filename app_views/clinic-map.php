...
$sql = "SELECT c.* FROM clinics c WHERE c.id = ?";
...
<button class="back-btn" style="        position: fixed;
        ...
        border-style: none;
         z-index: 10;
        background-color: #f1f1f1;">
    <a href="javascript:history.go(-1)" style=" width: 100%; text-decoration: none; cursor: pointer; color: #000;">
...
<section class="map">
    ...
    <?php
    if (! is_null($result['map'])) {
        echo $result['map'];
    } else {
        echo "<h2 style='margin-top: 115px; text-align: center'>Map not found!</h2>";
    }
