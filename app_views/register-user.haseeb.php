...
if (isset($_POST['referral'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($updatedReferralCount == 3) {
            ...
            $insertVoucher = "INSERT INTO vouchers (account_id, voucher_code) VALUES (?, ?)";
            $stmt = $con->prepare($insertVoucher);
            $stmt->bind_param("is", $accountId, $voucherCode);

            $stmt->execute();

            $resetReferralCount = "UPDATE accounts SET referral_count = 0 WHERE profilelevel = ?";
...
// Redirect to the landing page
        ...
        if ($stmt = $con->prepare('INSERT INTO accounts (username, password, email, activation_code, mobile, otp, userunique, userlevel, profilelevel, referral) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')) {
            ...
            if (isset($_POST['referral'])) {
                ...
                if ($_POST['referral'] && $result->num_rows > 0) {
                    $voucherCode = generateUniqueVoucherCode();

                    $insertVoucher = "INSERT INTO vouchers (account_id, voucher_code) VALUES (?, ?)";
                    $stmt = $con->prepare($insertVoucher);
                    $stmt->bind_param("is", $lastUserId, $voucherCode);
                    $stmt->execute();
                }
            }
            ...
            $currency = UserCountry() == 'Switzerland' ? 'CHF' : 'AED';
            $stmt_cuurency = $con->prepare('INSERT INTO user_currency (currency, user_id) VALUES (?, ?)');
            $stmt_cuurency->bind_param('si', $currency, $lastUserId);
            $stmt_cuurency->execute();
