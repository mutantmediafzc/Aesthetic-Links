...
$stmt2 = $con->prepare("UPDATE services SET book_max_clicks = ? WHERE id = ?");
$stmt2->bind_param("ii", $counter, $id);
...
<body>
    ...
    $getServiceViewStmt = "SELECT * FROM services_view WHERE service_id = '$cserviceId' AND date = '$dateToday'";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if ($serviceViewResult->num_rows > 0) {
                while ($serviceRow = $serviceViewResult->fetch_assoc()) {
                    $view = $serviceRow['views'] + 1;

                    $updateStmt = $con->prepare("UPDATE services_view SET views = ? WHERE id = ?");
                    $updateStmt->bind_param('ss', $view, $serviceRow['id']);
                    $updateStmt->execute();
                }
            } else {
                $clinic = $row['cname'];
                ...
                $service = $row['sname'];
                $createStmt = $con->prepare("INSERT INTO services_view (service_id, clinic_name, location, service_name, views, date) VALUES (?, ?, ?, ?, ?, ?)");
                $createStmt->bind_param('ssssss', $cserviceId, $clinic, $location, $service, $view, $dateToday);
                $createStmt->execute();
            }
    ...
    <?php
        ...
        <div class="expert-content" style=" display: grid; grid-template-columns: repeat(1, 1fr);">
            ...
            <div class="confirm-btn-box">
                <button class="book-now-calendar" id="confirm-expert" onclick="confirmSched()">Confirm</button>
            </div>
        </div>
     ...
     <div class="service-details-img-section">
        <div class="service-details-img-container">
            <button class="back-btn">
                <a href="clinic-details.blade.php?id=' . $cid . '&cunique=' . $cunique . '" style="height: max-content; width: 100%; text-decoration: none; cursor: pointer; color: #000;">
                    <i class="fa-solid fa-angle-left" id="back-arrow"></i>
                </a>
            </button>
            
            <img style="border-radius: 16px; margin-top: 100px; height: 248px" src="assets/images/services/' . $row['id'] . '.png">
        </div>

    </div>

   <div class="service-details-section" style="margin-top: 10px">
        <div class="service-name-box" style="display: flex; justify-content: space-between; align-items: center;">
            <h3 class="service-title" style="font-size: 16px; margin: 0; font-weight: 900;">' . $row['sname'] . '</h3>
            <p class="service-location" style="font-size: 10px; width: 20%!important; text-align: right; align-items: center;">
                <i class="fa-solid fa-stopwatch" style="margin-right: -2px;"></i> ' . $row['sduration'] . '
            </p>
        </div>
    </div>
    
    
   <div class="more-details-section" style="margin-top: 0">
        <div class="address">
            <div style="width: 100%; display: grid; grid-template-columns: repeat(2, max-content 90%);">
                <div style="width: 100%;">
                        ';

            if ($row['ratings_count'] >= 10) {
                echo '<a href="service-reviews.php?cunique=' . $row['cunique'] . '&id=' . $row['id'] . '" style="text-decoration: none; color: #212121;"><span class="service-rating" style="font-size: 12px; font-weight: 100;">' . $avgRating . ' <i class="fa-solid fa-star" id="service-star" style="color: #0CB4BF;"></i> <u>' . $row['ratings_count'] . ' reviews</u></span></a>';
            } else {
                echo '<a href="service-reviews.php?cunique=' . $row['cunique'] . '&id=' . $row['id'] . '" style="text-decoration: none; color: #212121;"><span class="service-rating" style="font-size: 12px; font-weight: 100;">' . $avgRating . ' <i class="fa-solid fa-star" id="service-star" style="color: #0CB4BF;"></i></span></a>';
            }

            echo '</div>
                <div style="width: 100%; display: flex; align-items: center;">
                    <div class="vertical-line"></div>
                    <p class="service-gender" style="margin: 0; font-size: 12px; color: #9a9c9d; font-weight: 100; font-weight: 100;">
                        <i class="fa-solid fa-venus-mars" style="margin-right: 5px"></i>' . $row['stype'] . '
                    </p>
                </div>
        ...
        <div class="pricing-container">
            <div style="width: 100%; visibility: hidden">
                    <p style="margin-top: 2px;" class="paragraph-4">from</p>
            </div>
            <div><h4 class="service-price-fee" style="font-size: 18px; font-weight: 900">' . convertCurrency($row['sprice']) . '</h4></div>
        </div>
    </div>

   <div class="consultation-fee-box" style="margin-top: 5px;">
        <h5 class="service-consultation-price" style="margin-left:20px; font-size: 11px; font-weight: normal;">
            Consultation fee (' . $row['sduration']  . ' min)<br>
            <span style="margin-left: 0px; font-size: 14px; font-weight: bold;">' . convertCurrency($row['scprice']) . '</span><br>
            <span style="font-weight: 400; font-size: 11px; color: #a1a1a1"><i>*Consulation from treatment cost if booked</i></span>
        </h5>
    </div>
    <div class="subcat" style="margin-top: 20px;">
        <div style="width: 100%;">
        <button disabled style="background-color: #e7e7e7; border: none; border-radius: 7px; width: auto; height: auto; padding: 4px 8px; min-width: 64px; min-height: 24px; color: #586271; font-size: 10px;">
            ' . $row['subcat'] . '
        </button>
        </div>
        <p style="font-size: 12px; margin-top: 15px; font-weight: 100;">' . $row['sbio'] . '</p>
    </div>
    ...
    <div style="left:0px; bottom:0px; height:auto; width:100%; display: flex; justify-content: center;">
        <div class="booking-btn-container">
            <a style="
                text-decoration: none;
                display: flex;
                justify-content: center;" href="#"><button class="book-now" style="border: none; font-size: 15px;" onclick="openCalendar()">Book now</button></a>
    
            <div style="width: 100%; height: auto; display: grid; grid-template-columns: repeat(3, 45% 10% 45%);">
                <div style="width: 100%; margin: auto;">
                    <hr class="solid" style="border-color: rgba(223,223,223,0.75)">
                </div>
                <div style="width: 100%; margin: auto;">
                    <p class="service-gender" style="text-align: center; color: #ccc">OR</p>
                </div>
                <div style="width: 100%; margin: auto;">
                    <hr class="solid" style="border-color: rgba(223,223,223,0.75)">
                </div>
            ...
            <div>
            <a href="book-now.php?cunique=' . $row['cunique'] . '&id=' . $cserviceId . '">
                <button style="width: 100%; height: auto; margin: 0; padding: 0; border-style: none; background-color: transparent;">
                    <p class="service-gender" style="color: #0C1E36; font-size: 16px; font-weight: 600"><u>Book consultation</u></p>
                </button>
                </a>
            </div>
