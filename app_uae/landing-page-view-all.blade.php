...
<div class="control-bar">
    ...
    <div class="control-bar-container">
        <div style="width: 100%;">
            <input id="search-box" type="text" class="search-bar-landing" placeholder="Search for Clinics...">

        </div>

        <div class="landing-page-filters">
            <a href="new-filter-page.blade.php<?php
                        $queryParameters = array();
                        if (isset($_POST['filters'])) {
                            $queryParameters['selectedFilters'] = urlencode(implode(",", $_POST['filters']));
                        }
                        if (isset($_POST['gender_preference'])) {
                            $queryParameters['gender_preference'] = $_POST['gender_preference'];
                        }
                        if (isset($_POST['price_preference'])) {
                            $queryParameters['price_preference'] = $_POST['price_preference'];
                        }
                        echo !empty($queryParameters) ? '?' . http_build_query($queryParameters) : '';
                    ?>">
            ...
            <div>
                ...
                <div id="myDropdown" class="dropdown-content radio-container">
                    ...
                    <input type="radio" name="sort" id="sort-reco" checked value="recommended" class="checkboxSort">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && ( isset($_POST['filters']) || isset($_POST['price_preference']) || isset($_POST['gender_preference']) )) {
        ...
        $stmt = "SELECT c.*, COUNT(r.id) as total_reviews, AVG(r.rating) AS average_rating FROM clinics c LEFT JOIN reviews r ON c.cunique = r.cunique COLLATE utf8mb4_unicode_ci JOIN services s ON c.cunique = s.cunique WHERE 1=1 AND c.ccountry = 'UAE' $ctypeCondition $priceCondition $selectedServicesCondition AND c.approval = 'approved' AND c.recommended = 1 GROUP BY c.cunique $havingClause";
    } else {
        $stmt = "SELECT c.*, COUNT(r.id) as total_reviews, AVG(r.rating) AS average_rating FROM clinics c LEFT JOIN reviews r ON c.cunique = r.cunique COLLATE utf8mb4_unicode_ci WHERE c.approval = 'approved' AND c.ccountry = 'UAE' AND c.recommended = 1 GROUP BY c.cunique";
    }
        ...
        <div class="card-item">
            ...
            <div class="text-container">
                ...
                <div class="second-line">
                    <div style="display:flex; align-items: center; justify-content: center; text-align:center;"> '. ' ' .  $imgClinic   .' <p class="paragraph-2">'  . $row['ctype'] . '</p></div>
                    <div style="display: flex; justify-content: center; align-items: center;"><i class="fa-solid fa-circle" id="circle"></i></div>
                    <div><p class="paragraph-2" style="color: #0CB4BF;">' . $avgRating . ' <i class="fa-solid fa-star" id="star"></i>&nbsp;';

                // Display review count only if it's greater than or equal to 10
                if ($row['total_reviews'] >= 10) {
                    echo '<span style="color: #212121;"><u>' . $row['total_reviews'] . ' reviews</u></span>';
                }
    ...
    <a href="landing-page.blade.php" style="font-family: 'poppinsregular'; width: 100%; text-decoration: none; color: #fff; width: 90%; margin: auto; font-family: 'poppinsregular';">
...
<script>
    ...
    // Close the modal if the user clicks or touches anywhere outside of it
