...
//              $sql = "SELECT * FROM appointments WHERE profilelevel = ? AND status = 'pending'";
              $sql = "SELECT * FROM appointments WHERE profilelevel = ? AND CONCAT(date, ' ', time) >= NOW()";
    ...
    <div id="History" class="tabcontent">
        <?php
            ...
            // Validate and query project details
            if ($userId) {
                $sql = "SELECT * FROM appointments WHERE profilelevel = ? AND CONCAT(date, ' ', time) <= DATE_SUB(NOW(), INTERVAL 1 HOUR)";
                $stmt = $con->prepare($sql);
              $stmt->bind_param("s", $userId);
              $stmt->execute();
              $result = $stmt->get_result();
