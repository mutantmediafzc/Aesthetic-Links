...
$sql2 = "SELECT calendar_id, cemail, clocation, ccity, ccountry FROM clinics WHERE cunique =  ?";
$stmt2 = $con->prepare($sql2);
$stmt2->bind_param("s", $clinic);
$stmt2->execute();
...
if ($serviceResult->num_rows>0) {
    ...
    if ($serviceViewResult->num_rows > 0) {
        while ($serviceRow = $serviceViewResult->fetch_assoc()) {
            $serviceSuccess = $serviceRow['success_no']+1;
            
            $updateStmt = $con->prepare("UPDATE services_view SET success_no = ? WHERE id = ?");
            $updateStmt->bind_param('ss', $serviceSuccess, $serviceRow['id']);
            $updateStmt->execute();
        }
    } else {
        $clinic = $row['cname'];
        ...
        $service = $row['sname'];
        $createStmt = $con->prepare("INSERT INTO services_view (service_id, clinic_name, location, service_name, success_no, date) VALUES (?, ?, ?, ?, ?, ?)");
        $createStmt->bind_param('ssssss', $service_id, $clinic, $location, $service, $serviceSuccess, $dateToday);
        $createStmt->execute();
    }
...
if ($clinicResult->num_rows>0) {
    ...
    if ($getClinicViewResult->num_rows > 0) {
        while ($clinicRow = $getClinicViewResult->fetch_assoc()) {
            $clinicSuccess = $clinicRow['success_no']+1;
            
            $updateStmt = $con->prepare("UPDATE clinics_view SET success_no = ? WHERE id = ?");
            $updateStmt->bind_param('ss', $clinicSuccess, $clinicRow['id']);
            $updateStmt->execute();
        }
    } else {
        $clinic = $row['cname'];
        $location = $row['ccity'].', '. $row['ccountry'];
        $createStmt = $con->prepare("INSERT INTO clinics_view (clinic_id, clinic_name, location, success_no, date) VALUES (?, ?, ?, ?, ?)");
        $createStmt->bind_param('sssss', $clinicId, $clinic, $location, $clinicSuccess, $dateToday);
        $createStmt->execute();
    }
              ...
              <div style="background-color: #fff; border-radius: 20px; max-width: 600px; margin: 0 auto;">
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" width="100%" style="border-collapse: collapse;">
                  ...
                  <tr>
                    ...
                    <td style="padding-right: 24px; color: #98C3BC; font-size: 16px; text-align: right;">
                        
                        <a style="padding-right: 24px; padding-bottom: 24px; font-weight: 600; color: #46AFB4; font-size: 18px; margin: 0; text-align: right;color: #46AFB4; text-decoration: none;" href="'.$clocation.'">Click Here</a>
...
<section class="book-now-section" style="position:fixed; left:0px; bottom:0px; height:auto; width:100%; display: flex; justify-content: center;">
    <div class="booking-btn-container">
        <div>
            <a href="myappointments.php?profilelevel=<?=$profilelevel?>"><div><button class="book-now">View Appointments</button></div></a>
            <button style="width: 100%; height: auto; margin: 0; padding: 0; border-style: none; background-color: transparent;" onclick="window.location.href='discover-page.blade.php'">
