...
<script>
    ...
    function openTime() {
  var nowdateElement = document.getElementById('today_date');
  var dateInputElement = document.getElementById('date-input');

  // Check if required elements exist
  if (!nowdateElement || !dateInputElement) {
    console.error("Missing required elements: today_date or date-input");
    return; // Exit the function if elements are missing
  }

  var nowdateString = nowdateElement.value;
  var dateString = dateInputElement.value;

  // Validate date format (consider using a date library for robust validation)
  // ... (add date validation logic here)

  var date1 = new Date(dateString);
  var date2 = new Date(nowdateString);
  date2.setDate(date2.getDate() + 2); // Two days from nowdate

  console.log(date1, date2);

  // Check if booking date is after or equal to the minimum allowed date
  if (date1 >= date2) {
    console.log("Valid booking date");
    // Open time modal logic
    var calendarmodal = document.getElementById('calendarModal');
    var timemodal = document.getElementById('timeModal');
    calendarmodal.style.display = 'none';
    timemodal.style.display = 'block';
  } else {
    // Show error message (consider creating the element if it doesn't exist)
    var errorElement = document.getElementById('error_message');
    if (errorElement) {
      errorElement.style.display = "block";
    } else {
      console.error("Error element (error_message) not found");
    }
  }
}
