...
<section class="container my-5">
    ...
    <div class="form-check">
        <input class="form-check-input" type="radio" name="gender_preference" id="radio1" value="all">
        <label class="form-check-label" for="radio1">All</label>
    </div>
    <div class="form-check">
        <input class="form-check-input" type="radio" name="gender_preference" id="radio2" value="male">
        <label class="form-check-label" for="radio2">Male</label>
    </div>
    <div class="form-check">
        <input class="form-check-input" type="radio" name="gender_preference" id="radio3" value="female">
            ...
            while ($row = $result->fetch_assoc()) {
                echo '
                    <div class="col-md-4 checkbox-item">
                        <input type="checkbox" name="filters[]" id="' . $row['subcat'] . '" value="' . $row['subcat'] . '" class="form-check-input">
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const genderPref = params.get('gender_preference');
        if (genderPref) {
            document.querySelector(`input[name="gender_preference"][value="${genderPref}"]`).checked = true;
        ...
        if (filters) {
            ...
            filterArray.forEach(filter => {
                const checkbox = document.querySelector(`input[name="filters[]"][value="${filter}"]`);
                if (checkbox) {
    ...
    function setGenderPreference() {
        var genderPreference = document.querySelector('input[name="gender_preference"]:checked').value;
    ...
    var radioButtons = document.querySelectorAll('input[name="gender_preference"]');
